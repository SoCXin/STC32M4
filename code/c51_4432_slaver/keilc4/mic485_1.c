...
void main(void)
{
	...
	lcd_printf_string(" slaver ", ROW(0x00)|LEN(0x08)|COL(0x00));
   ...
   SpiWriteRegister(0x07, 0x80);															//write 0x80 to the Operating & Function Control1 register 
    ...
    SpiWriteRegister(0x72, 0x38);	//(1.2kbps)															//write 0x48 to the Frequency Deviation register 
  ...
  //SpiWriteRegister(0x0C, 0x12);															//write 0x12 to the GPIO1 Configuration(set the TX state)
	...
	SpiWriteRegister(0x0b, 0x12);															//write 0x12 to the GPIO0Configuratit the RX state) 
			...
			if( (ItStatus1 & 0x02) == 0x02 )
			{
				...
				if(length < 11)
				{
					...
					if( length == 8 ) 
	                {
	                      ...
	                      lcd_printf_string(temp_array, ROW(0x01)|LEN(0x08)|COL(0x00));
	                      lcd_printf_string("rx:     ", ROW(0x00)|LEN(0x08)|COL(0x00));
					}
				}
       	}
			//reset the RX FIFO
